...
(Ns = 50 staggered sites = 100 system qubits + 1 ancilla) on IBM Heron r3
(heavy-hex) and Nighthawk (square lattice).  No dependency on the research
code (`htensor`); `tests/test_htq_hw_crosscheck.py` in the repository root
proves equality against it.  Nothing in this package submits to IBM unless
`submit --real` is invoked explicitly.
...
PYTHONPATH=. python -m htq_hw --ns 50 check --target <backend> --times 0.5 1.0 --ideal <grids>
PYTHONPATH=. python -m htq_hw --ns 50 rehearse --target <backend> --shots-scale 0.01
```
...
- Nighthawk repetition rate: the 36-minute manifest at 250 us per shot becomes 9.6 h at 4 ms;
  either a faster rep rate, a reduced manifest (t <= 3, `--times`), or a larger allocation.
- Access to ibm_boston (or another Heron r3) and ibm_miami (Nighthawk) for the CZ and fractional
  sessions, with the calibration snapshot at run time.
...
PYTHONPATH=. python -m htq_hw submit --target fake:boston --fetch          # local testing mode
PYTHONPATH=. python -m htq_hw submit --target ibm_boston --real            # the real thing
PYTHONPATH=. python -m htq_hw fetch data/hw/htq_job_<id>.json
PYTHONPATH=. python -m htq_hw analyze data/hw/htq_bits_<id>.npz --ideal "..."
PYTHONPATH=. python -m htq_hw bundle --report-json report.json
```
